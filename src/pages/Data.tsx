...
import { Meter, Reveal, SectionHead } from '../components/ui'
import {
  GAMBARAN_UMUM,
  KEPESERTAAN_KB,
  PENDIDIKAN,
...
export default function DataStatistik() {
  ...
  const rtKK = jumlah(PENDUDUK_RT, (r) => r.kk)
  const rtJiwa = jumlah(PENDUDUK_RT, (r) => r.jiwa)
  const adaSelisih = rtKK !== TOTAL_RESMI.kk || rtJiwa !== TOTAL_RESMI.jiwa

  const didikL = jumlah(PENDIDIKAN, (r) => r.l)
  const didikP = jumlah(PENDIDIKAN, (r) => r.p)
  const didikTotal = didikL + didikP

  const totalPus = jumlah(KEPESERTAAN_KB, (r) => r.jml)

  const ringkas: { icon: IconName; v: string; l: string }[] = [
    { icon: 'users', v: angka(GAMBARAN_UMUM.penduduk), l: 'Jiwa penduduk' },
    { icon: 'home', v: angka(GAMBARAN_UMUM.kk), l: 'Kepala keluarga' },
    { icon: 'layers', v: '1.818,00 ha', l: 'Luas wilayah' },
    { icon: 'building', v: '16 / 3', l: 'RT / RW' },
    { icon: 'rings', v: angka(GAMBARAN_UMUM.pus), l: 'Pasangan usia subur' },
    { icon: 'map-pin', v: '± 9 km', l: 'Jarak ke pusat kota' },
  ]

  const sasaran: { icon: IconName; v: number; l: string }[] = [
    { icon: 'baby', v: GAMBARAN_UMUM.balitaStunting, l: 'Balita gizi kurang / stunting' },
    { icon: 'heart', v: GAMBARAN_UMUM.ibuHamil, l: 'Ibu hamil terdata' },
    { icon: 'book', v: GAMBARAN_UMUM.remaja, l: 'Remaja' },
    { icon: 'users', v: GAMBARAN_UMUM.lansia, l: 'Lanjut usia' },
  ]
      ...
      <section className="section section--cream">
        <div className="container">
          <SectionHead
            ...
            lead="Rincian jumlah kepala keluarga dan jiwa pada 16 RT yang tersebar di 3 RW."
          />
          <Reveal>
          ...
          {adaSelisih && (
            <Reveal>
              <div className="form-alert" style={{ marginTop: '1.25rem' }}>
                <Icon name="info" />
                <span>
                  Angka pada baris <strong>Jumlah baris</strong> adalah hasil
                  penjumlahan tabel di atas. Total yang dipublikasikan pada
                  profil Kampung KB berbeda, yaitu{' '}
                  <strong>{angka(TOTAL_RESMI.kk)} KK</strong> dan{' '}
                  <strong>{angka(TOTAL_RESMI.jiwa)} jiwa</strong>. Selisih ini
                  berasal dari sumbernya dan ditampilkan apa adanya — mohon
                  dikonfirmasi ke kelurahan pada pemutakhiran berikutnya.
                </span>
              </div>
            </Reveal>
          )}
          ...
          <div className="grid-2" style={{ marginTop: '1.5rem' }}>
            <Reveal>
              <div className="panel" style={{ height: '100%' }}>
                ...
                <p className="panel__sub">
                  Dari {angka(totalPus)} pasangan usia subur yang tercatat pada
                  blok Statistik Kampung.
                </p>
                <div className="meter-stack">
                  {KEPESERTAAN_KB.map((k) => (
                    <Meter
                      key={k.name}
                      name={`${k.name} — ${angka(k.jml)}`}
                      pct={Number(((k.jml / totalPus) * 100).toFixed(1))}
                      color={k.color}
                    />
                  ))}
                </div>
            ...
            <Reveal delay={90}>
              <div className="panel panel--forest" style={{ height: '100%' }}>
                <p className="panel__title">Keluarga dalam kelompok kegiatan</p>
                <p className="panel__sub">
          ...
          <Reveal>
            <div className="form-alert" style={{ marginTop: '1.25rem' }}>
              ...
              <span>
                ...
                tabel Gambaran Umum — {angka(STATISTIK_KAMPUNG.jiwa)} jiwa dan{' '}
                {angka(STATISTIK_KAMPUNG.pus)} PUS, dibanding{' '}
                {angka(GAMBARAN_UMUM.penduduk)} jiwa dan{' '}
                {angka(GAMBARAN_UMUM.pus)} PUS. Keduanya ditampilkan terpisah
              ...
              <Reveal key={g.grup} delay={(i % 4) * 70}>
                <div className="panel" style={{ height: '100%' }}>
                  ...
                  <div style={{ marginTop: '0.75rem' }}>
                    {g.items.map((it) => (
                      <div key={it.name} className="datarow">
                        <span className="datarow__key">{it.name}</span>
                        <span
        ...
        <div className="container">
          ...
          <Reveal>
            <div className="table-wrap">
              <div className="table-scroll">
                <table className="data-table">
                  <caption className="sr-only">
                    Daftar posyandu aktif beserta alamat dan wilayah layanannya
                  </caption>
                  <thead>
                    <tr>
          ...
          <Reveal>
            <div className="form-alert" style={{ marginTop: '1.75rem' }}>
              <Icon name="info" />
              <span>
                Seluruh angka pada halaman ini bersumber dari {SUMBER_DATA} dan
                bersifat sementara sampai dilakukan pemutakhiran berikutnya.
                Untuk keperluan penelitian atau permintaan data resmi, silakan
                ajukan melalui halaman Kontak.
              </span>
            </div>
          </Reveal>
        </div>
